...
function Sidebar({ isOpen, tabs }) {
    ...
    const handleTabClick = (tab) => {
        if (tab.submenus && tab.submenus.length > 0) {
            setActiveSubmenus(activeSubmenus.includes(tab.name) ? activeSubmenus.filter(name => name !== tab.name) : [...activeSubmenus, tab.name])
        } 
        else {
            navigate(tab.path)
        }
                ...
                {isSubmenuActive && (
                    <div className="submenu-container">
                        {tab.submenus.map((submenu) => {
                            ...
                            return (
                                <NavLink to={submenu.path} key={submenu.name} className={`sidebar-submenu-navlinks ${isSubmenuItemSelected ? 'active' : ''}`} style={{ color: colorStyle }} >
