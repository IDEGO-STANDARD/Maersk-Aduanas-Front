    ...
    const renderedKeys = data?.map((elem, index) => {
        ...
        return (<div key={index} style={{minWidth: minwidth}} className="dd-rendered-keys-key-cont">
            <div className="dd-rendered-keys-top-cont">
                ...
                {elem.hasOwnProperty('checked') && edit && <input className="dd-rendered-keys-checkbox" onChange={(e) => {handleChangeData(elem.name, e.target.checked, elem.value, documentid)}} type="checkbox" checked={elem.checked}/>}
            </div>
